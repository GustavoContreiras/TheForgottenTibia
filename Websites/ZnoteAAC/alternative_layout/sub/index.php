...
//////////////////////////////////////////////
// Load slideshow box and its configuration //
include 'layout/config_slide.php'; ///////////
//////////////////////////////////////////////
	...
	if ($view !== "") { // We want to view a specific news post
		...
		for ($i = $current; $i < $current + $config['news_per_page']; $i++) {
			if (isset($news[$i])) {
				...
				<?php echo ' by <a href="characterprofile.php?name='. $news[$i]['name'] .'"><a href="?view='.urlencode($news[$i]['title']).'">'. $news[$i]['name'] .'</a> - <b>'. TransformToBBCode($news[$i]['title']) .'</b></a>'; ?>
						<div class="date"><?php echo getClock($news[$i]['date'], true); ?></div>
				</div>
