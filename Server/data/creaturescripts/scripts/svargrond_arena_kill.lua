function onKill(creature, target)
	...
	local pit = player:getStorageValue(SvargrondArena.Storages.Pit)
	if pit < 1 or pit > 10 then
		return
	end

	local arena = player:getStorageValue(SvargrondArena.Storages.Arena)
	if arena < 1 then
		return
	end
	...
	player:setStorageValue(SvargrondArena.Storages.Pit, pit + 1)
