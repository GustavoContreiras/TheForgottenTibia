...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:addItemEx(reward) ~= RETURNVALUE_NOERROR then
		...
		if player:getFreeCapacity() < weight then
			player:sendCancelMessage(string.format('You have found %s weighting %.2f oz. You have no capacity.', result, (weight / 100)))
		else
			player:sendCancelMessage('You have found ' .. result .. ', but you have no container to take it.')
		end
		return true
	end
