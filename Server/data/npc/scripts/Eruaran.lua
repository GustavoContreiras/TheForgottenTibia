...
noitemss        = 'You do not have all the required items.' 
noitemss2    = 'You do not have all the required items or you do not have the first addon, which by the way, is a requirement for this addon.' 
alreadyy        = 'It seems you already have this addon, don\'t you try to mock me son!'
function CitizenFirst(cid, message, keywords, parameters, node) 
    ...
    addon = getPlayerStorageValue(cid,22149) 
    if addon == -1 then 
        ...
        if doPlayerRemoveItem(cid,22610,1) then 
            ...
            setPlayerStorageValue(cid,22149,1) 
        end 
        else 
            selfSay(noitemss, cid) 
        end 
    else 
        selfSay(alreadyy, cid) 
    end 
    end 
...
function CitizenSecond(cid, message, keywords, parameters, node) 
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,22149+1) 
    if addon == -1 then 
        ...
        if doPlayerRemoveItem(cid,22609,1) then 
            ...
            setPlayerStorageValue(cid,22149+1,1) 
        end 
        else 
            selfSay(noitemss, cid) 
        end 
...
-- OUTFIT END --
keywordHandler:addKeyword({'outfit'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "to make the mask of the outfit speak {first dream warden}, to make the claw say {second dream warden}."}) 

 node1 = keywordHandler:addKeyword({'first dream warden'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first dream warden addon you need to give me Dream Wardem Mask. Do you have them with you?'}) 
    node1:addChildKeyword({'yes'}, CitizenFirst, {}) 
    node1:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true}) 

 node2 = keywordHandler:addKeyword({'second dream warden'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first dream warden addon you need to give me Dream Wardem Claw. Do you have them with you?'}) 
