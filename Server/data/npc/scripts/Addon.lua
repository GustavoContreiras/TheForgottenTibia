...
function YalaharianFirst(cid, message, keywords, parameters, node) 
    ...
    if addon == -1 then 
        if getPlayerItemCount(cid,9955) >= 1 then 
        if doPlayerRemoveItem(cid,9955,1) then 
            selfSay(newaddon, cid) 
...
-- DEMONHUNTER START -- 
function DemonhunterFirst(cid, message, keywords, parameters, node) 
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,demonhunter) 
    if addon == -1 then 
...
function DemonhunterSecond(cid, message, keywords, parameters, node) 
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,demonhunter+1) 
    if addon == -1 then 
        if getPlayerItemCount(cid,5906) >= 50 and getPlayerItemCount(cid,6500) >= 200 and getPlayerItemCount(cid,2151) >= 100 then 
        if doPlayerRemoveItem(cid,5906,40) and doPlayerRemoveItem(cid,6500,200) and doPlayerRemoveItem(cid,2151,100) then 
            selfSay(newaddon, cid) 
              
            doSendMagicEffect(getCreaturePosition(cid), 13) 
            doPlayerAddOutfit(cid, 289, 2) 
            doPlayerAddOutfit(cid, 288, 2) 
            setPlayerStorageValue(cid,demonhunter+1,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
function testAddon(cid, message, keywords, parameters, node) 
...
keywordHandler:addKeyword({'help'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "What you do is that you type 'first hunter addon' or 'second hunter addon' where you replace 'hunter' with whatever outfit you'd like to achieve the addons for. Assuming that you already collected all the required pieces, say 'yes' and voíla - you got yourself an addon!"}) 
 ...
 node18 = keywordHandler:addKeyword({'second druid addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second druid addon you need to give me Botanist Container, Ceirons waterskin, 100 demon dusts, a blessed wooden stake and Ceirons wolf tooth chain. Do you have them with you?'}) 
 ...
 node38 = keywordHandler:addKeyword({'second brotherhood addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second brotherhood addon you need to give me 1000 demonic essences. Do you have them with you?'}) 
 ...
 node39 = keywordHandler:addKeyword({'first yalaharian addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first yalaharian addon you need to give me a vampiric crest. Do you have them with you?'}) 
 ...
 node40 = keywordHandler:addKeyword({'second yalaharian addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second yalaharian addon you need to give me a vampiric crest. Do you have them with you?'}) 
