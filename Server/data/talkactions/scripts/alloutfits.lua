...
function onSay(player, words, param)
	...
	if param == '' then
		...
		if not target then
			player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, 'Gives players the ability to wear all outfits. Usage: /alloutfis <player name>')
			return false
		end
	else
		target = Player(param)
	...
	for i = 1, #looktypes do
		target:addOutfitAddon(looktypes[i], 0)
	end

	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, 'All outfits unlocked for ' .. target:getName() .. '.')
	target:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, '[Server] All outfits unlocked.')
	return false
end
